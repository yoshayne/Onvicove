...
function CalendarFirstLayout({ services, currency, paymentsEnabled, accentColor, textColor, surfaceColor, onBook }: {
  services: ServiceData[]; currency?: string; paymentsEnabled?: boolean;
  ...
  const border = `${textColor ?? '#111'}18`;

  return (
    <div className="flex flex-col lg:flex-row gap-8">
      {/* Calendar */}
      <div className="lg:w-80 shrink-0">
        <div className="rounded-2xl overflow-hidden border" style={{ borderColor: border, background: surfaceColor }}>
          {/* Month nav */}
          <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: border }}>
            ...
            <p className="font-semibold text-sm" style={{ color: textColor }}>{monthName}</p>
          ...
          <div className="grid grid-cols-7 px-3 pt-3">
            {['Su','Mo','Tu','We','Th','Fr','Sa'].map(d => (
              <div key={d} className="text-center text-xs font-medium opacity-40 pb-2" style={{ color: textColor }}>{d}</div>
            ))}
          </div>
          {/* Day cells */}
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    color: selected ? '#fff' : past ? `${textColor ?? '#111'}30` : textColor,
        ...
        {selectedDay && (
          <p className="text-center text-xs opacity-50 mt-3" style={{ color: textColor }}>
            Select a service to book for {monthName.split(' ')[0]} {selectedDay}
          </p>
        )}
      </div>

      {/* Service list — shown once a day is picked, or always */}
      <div className="flex-1">
        {!selectedDay ? (
          <div className="flex items-center justify-center h-full min-h-32 opacity-40 text-sm" style={{ color: textColor }}>
            Pick a date to see availability
          </div>
        ) : (
          <div className="flex flex-col gap-3">
            {services.map((s) => (
              <div key={s.id} className="flex items-center gap-4 rounded-xl px-5 py-4 border"
                style={{ borderColor: border, background: surfaceColor }}>
                {s.imageUrls?.[0] && (
                  <div className="w-14 h-14 shrink-0 rounded-lg overflow-hidden">
                    <img src={s.imageUrls[0]} alt={s.name} className="w-full h-full object-cover" />
                  </div>
                )}
                <div className="flex-1 min-w-0">
                  <p className="font-semibold" style={{ color: textColor }}>{s.name}</p>
                  <p className="text-xs opacity-50 mt-0.5" style={{ color: textColor }}>{s.durationMinutes} min</p>
                  {s.description && <p className="text-sm opacity-60 mt-1 line-clamp-2">{s.description}</p>}
                </div>
                <div className="shrink-0 text-right flex flex-col items-end gap-2">
                  <p className="font-bold text-sm" style={{ color: accent }}>{formatPrice(s.priceCents, currency)}</p>
                  <button type="button" onClick={() => onBook(s)} disabled={!paymentsEnabled}
                    ...
                    {paymentsEnabled ? 'Book' : 'Coming Soon'}
                  </button>
                </div>
              </div>
            ))}
...
export function ServiceCatalog({
  ...
  if (effectiveLayout === 'calendar-first') {
    return (
      <CalendarFirstLayout
        services={services}
        currency={currency}
