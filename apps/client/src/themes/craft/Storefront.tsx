...
function ServiceDesc({ desc }: { desc: string }) {
  ...
  if (desc.length <= LIMIT) {
    return <p style={{ fontSize: 13, color: '#7a6650', marginBottom: 20, lineHeight: 1.6 }}>{desc}</p>;
  }
  return (
    <p style={{ fontSize: 13, color: '#7a6650', marginBottom: 20, lineHeight: 1.6 }}>
      {expanded ? desc : desc.slice(0, LIMIT) + '…'}
      {' '}
      <button onClick={() => setExpanded(!expanded)} style={{ fontSize: 11, color: '#5c4a32', background: 'none', border: 'none', cursor: 'pointer', padding: '2px 0', textDecoration: 'underline' }}>
...
export default function Storefront({ theme, products, services, staff, visibleSections, galleries }: ThemeProps) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f5f0e8', color: '#2c1f14', fontFamily: 'Inter, sans-serif' }}>
      ...
      {/* Hero — split layout */}
      {isVis('hero') && (
      <section style={{ order: secOrder('hero'), maxWidth: 1280, margin: '0 auto', padding: '64px 24px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center', minHeight: '65vh' }}>
        <div>
          {/* Stamp badge */}
      ...
      {/* Services / Workshops */}
      {showServices && (
        <section id="services" style={{ order: secOrder('services'), background: '#ece5d8', padding: '64px 24px', borderTop: '1px solid #c4b49a' }}>
          <div style={{ maxWidth: 1280, margin: '0 auto' }}>
      ...
      <footer id="footer" style={{ background: '#2c1f14', color: '#f5f0e8', padding: '48px 24px', textAlign: 'center' }}>
        ...
        <ContactBlock theme={theme} useInlineStyles textColor="rgba(245,240,232,0.5)" />
